...
function acx_fsmi_show_purchased_license_html()
{
	...
	<h4><?php _e("Purchased Addons List",ACX_FSMI_WP_SLUG); ?></h4>
	<table class="wp-list-table widefat fixed striped">
		<tr>
			<th class="manage-column"><?php _e('Plugin Name',ACX_FSMI_WP_SLUG);?></th>
			<th class="manage-column"><?php _e('Licence Code',ACX_FSMI_WP_SLUG);?></th>
			<th class="manage-column"><?php _e('Version',ACX_FSMI_WP_SLUG);?></th>
			<th class="manage-column"><?php _e('Licence Status',ACX_FSMI_WP_SLUG);?></th>
			<th class="manage-column"><?php _e('Plugin Status',ACX_FSMI_WP_SLUG);?></th>
			<th class="manage-column"><?php _e('Action',ACX_FSMI_WP_SLUG);?></th>
		</tr>
	...
	{
		?>
		<tr><td colspan="5"><?php _e('No Licence found',ACX_FSMI_WP_SLUG);?></td></tr>
		<?php
	}
	...
	foreach($acx_fsmip_licence_array as $key => $value)
	{
		...
		if(ISSET($acx_plugin_array['slug']) && ISSET($acx_plugin_array['download_dynamic_url']))
		{
			...
			{
				$acx_fsmi_plugin_stat = __('Active',ACX_FSMI_WP_SLUG);
			}
			else 
			{
				$acx_fsmi_plugin_stat =  __('Not Active <a href="'.wp_nonce_url(admin_url("admin.php?page=Acurax-Social-Icons-Settings-Install-Addons")).'">[Install Now]</a>',ACX_FSMI_WP_SLUG);
			}
		}
		...
		<td data-licence="<?php echo $value['licence_code']; ?>" data-key="<?php echo $key;?>"><span class="ax_fsmi_action_icon acx_fsmi_lic_refresh <?php echo $acx_refresh_stat; ?>" title="<?php _e("Refresh",ACX_FSMI_WP_SLUG);?>"></span><span class="ax_fsmi_action_icon acx_fsmi_lic_del" title="<?php _e("Delete",ACX_FSMI_WP_SLUG);?>"></span></td>
		</tr>
		<?php
	}
	?>
...
function acx_fsmi_licnece_install_required_plugins() {
	...
	foreach($acx_fsmip_licence_array as $key => $value)
	{
		...
		$config = array(
			...
			'dismiss_msg'  => '',
			'is_automatic' => true,
			'message'      => '<h4>'.__('Install ',ACX_FSMI_WP_SLUG).$value['addon_name'].'</h4>',
			'strings'      => array(
			'notice_can_install_required'     => _n_noop(
...
function acx_fsmi_show_license_add_button_html()
{
	...
	<span class="acx_fsmi_add_p_licen"><a class="acx_fsmi_purchas_lic_add button button-primary"><?php _e('CLICK HERE TO ADD LICENCE',ACX_FSMI_WP_SLUG);?></a></span><!-- acx_fsmi_add_p_licen -->
	...
	<div class="acx_fsmi_add_p_licence_light_content">
		...
		<input type="text" id="acx_fsmi_purchased_licence" placeholder="<?php _e('Enter Your Licence Code',ACX_FSMI_WP_SLUG);?>" value=""/>
		<span class="acx_fsmi_pu_li_error"></span>
		<a class="acx_fsmi_add_p_li_cls button button-primary"><?php _e('SUBMIT',ACX_FSMI_WP_SLUG);?></a>
	...
	jQuery(".acx_fsmi_add_p_li_cls").click(function()
	{
		...
		{
			jQuery(".acx_fsmi_pu_li_error").html('<?php _e('Invalid Licence Code',ACX_FSMI_WP_SLUG);?>');
			return false;
		}
		...
		jQuery.post(ajaxurl, acx_fsmip_order, function(theResponse)
		{
			...
			{
				alert('<?php _e('Successfully Added !!',ACX_FSMI_WP_SLUG);?>');
	...
	{
		var acx_confirm = confirm("<?php _e('Are you sure to delete this license?',ACX_FSMI_WP_SLUG)?>");
		if(acx_confirm == true)
		{
			...
			jQuery.post(ajaxurl, acx_fsmip_del_order, function(theResponse)
			{
				...
				if(theResponse == "success")
				{
					alert('<?php _e('Successfully deleted the licence !!',ACX_FSMI_WP_SLUG);?>');
					var link = window.location.href;
					if(link !== '')
					{
						window.location=link;
	...
	{
		var acx_confirm = confirm("<?php _e('Are you sure to refresh this license?',ACX_FSMI_WP_SLUG)?>");
		if(acx_confirm == true)
		{
			...
			jQuery.post(ajaxurl, acx_fsmip_refresh_order, function(theResponse)
			{
				...
				if(theResponse == "success")
				{
					alert('<?php _e('Successfully refreshed the license !!',ACX_FSMI_WP_SLUG);?>');
					var link = window.location.href;
					if(link !== '')
					{
						window.location=link;
					...
					alert('<?php _e('Failed To Refresh The Licence !!',ACX_FSMI_WP_SLUG);?>');
				}
			});
		}
	});	
...
{
	if (!isset($_POST['acx_fsmi_install_licence_del_w_c_n'])) die("<br><br>".__('Unknown Error Occurred, Try Again... ',ACX_FSMI_WP_SLUG)."<a href=''>".__('Click Here',ACX_FSMI_WP_SLUG)."</a>");
	if (!wp_verify_nonce($_POST['acx_fsmi_install_licence_del_w_c_n'],'acx_fsmi_install_licence_del_w_c_n')) die("<br><br>".__('Unknown Error Occurred, Try Again... ',ACX_FSMI_WP_SLUG)."<a href=''>".__('Click Here',ACX_FSMI_WP_SLUG)."</a>");
...
{
	if (!isset($_POST['acx_fsmi_purchased_licence_w_c_n'])) die("<br><br>".__('Unknown Error Occurred, Try Again... ',ACX_FSMI_WP_SLUG)."<a href=''>".__('Click Here',ACX_FSMI_WP_SLUG)."</a>");
	if (!wp_verify_nonce($_POST['acx_fsmi_purchased_licence_w_c_n'],'acx_fsmi_purchased_licence_w_c_n')) die("<br><br>".__('Unknown Error Occurred, Try Again... ',ACX_FSMI_WP_SLUG)."<a href=''>".__('Click Here',ACX_FSMI_WP_SLUG)."</a>");
	...
	if($acx_fsmi_purchased_licence != "")
	{
		if (strpos($acx_fsmi_purchased_licence, 'FSMI') !== false) 
		{
			...
			else
			{
				...
				{
					echo __("Sorry, Your license validation failed due to ".$response['response_status'].", Please contact support",ACX_FSMI_WP_SLUG);
				}
			}
		}
		else
		{
			echo __("Sorry, Your license code is invalid, Please contact support",ACX_FSMI_WP_SLUG);
		}
	}
	else
	{
		echo __("Licence Code is Empty",ACX_FSMI_WP_SLUG);
	} 
	die();
}
